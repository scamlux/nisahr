...
export default function ProgressPage() {
  ...
  if (isLoading || !data) {
    return (
      <div className="mx-auto max-w-6xl px-4 py-8 lg:px-8">
        <PageHeader title="Progress" subtitle="Loading your analytics…" />
  ...
  const kpis = [
    { icon: Flame, label: 'Day streak', value: data.streakDays, color: 'text-warning', suffix: '' },
    { icon: Clock, label: 'Hours this week', value: data.weeklyHours, color: 'text-primary', suffix: 'h', decimals: 1 },
    { icon: GraduationCap, label: 'Skills completed', value: data.completedSkills, color: 'text-accent', suffix: '' },
    { icon: Award, label: 'Courses done', value: data.completedCourses, color: 'text-success', suffix: '' },
  ];

  return (
    <div className="mx-auto max-w-6xl px-4 py-8 lg:px-8">
      <PageHeader title="Progress & Analytics" subtitle="Your momentum, at a glance." />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card p-5 lg:col-span-2">
          <p className="mb-4 flex items-center gap-2 text-sm font-medium"><TrendingUp className="h-4 w-4 text-primary" /> Weekly learning hours</p>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card flex flex-col items-center justify-center gap-3 p-5">
          <p className="text-sm font-medium">Roadmap completion</p>
          <ProgressRing value={data.roadmapCompletion} sublabel="done" />
          <p className="text-center text-xs text-muted">{data.monthlyHours}h logged in the last 30 days</p>
        </motion.div>
      </div>

      {/* heatmap */}
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card mt-6 p-5">
        <p className="mb-4 text-sm font-medium">This week’s activity</p>
      ...
      {/* AI insights */}
      <div className="mt-6">
        <p className="mb-3 flex items-center gap-2 text-sm font-medium"><Brain className="h-4 w-4 text-primary" /> AI learning insights</p>
        {!isPremium ? (
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="grid gap-4 md:grid-cols-2">
            <div className="card p-5">
              <p className="flex items-center gap-2 text-sm font-medium"><Sparkles className="h-4 w-4 text-accent" /> Weekly summary</p>
              <p className="mt-2 text-sm text-muted">{insights.weeklySummary}</p>
              <div className="mt-4 flex flex-wrap gap-2 text-xs">
                <span className="chip">Pace: {insights.pace}</span>
                <span className={`chip ${insights.motivationRisk === 'HIGH' ? 'border-danger/30 text-danger' : insights.motivationRisk === 'MEDIUM' ? 'border-warning/30 text-warning' : 'border-success/30 text-success'}`}>
                  Motivation risk: {insights.motivationRisk}
                </span>
              </div>
            </div>
            <div className="card p-5">
              <p className="text-sm font-medium">Growth zones</p>
