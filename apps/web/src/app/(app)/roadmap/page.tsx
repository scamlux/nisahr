    ...
    <div className="mx-auto max-w-6xl px-4 py-8 lg:px-8">
      ...
      {!roadmaps?.length ? (
        <EmptyState
          ...
          action={<button className="btn-primary" onClick={() => setShowGen(true)}><Sparkles className="h-4 w-4" /> {t.pages.roadmap.generateRoadmap}</button>}
        />
      ) : (
        <>
          {/* roadmap switcher */}
                  ...
                  <div className="mt-4 space-y-2 border-t border-border/60 pt-4">
                    {stage.tasks.map((task: any) => (
                      <label key={task.id} className="flex cursor-pointer items-start gap-3 text-sm">
                        <button
                          type="button"
                          onClick={() => taskMut.mutate(task.id)}
                          className={cn(
